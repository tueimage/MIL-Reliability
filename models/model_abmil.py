...
class MADMIL_CLASS(nn.Module):
    ...
    def forward(self, h, attention_only=False):
        ...
        # Multihead Input
        h = h.reshape(N, self.n_heads, self.step) # Nxheadsxstep
        A = torch.stack([self.attention_net[nn](h[:,nn,:])[0] for nn in range(self.n_heads)], dim=1) # NxheadsxK
        ...
        # Multihead Output
        M = torch.einsum('ijk,kjl->ijl', A, h) # Kxheadsxstep
        ...
        logits = torch.sum(outputs, dim=0, keepdim=True) # 1x2
        Y_hat = torch.topk(logits, 1, dim=1)[1]
        Y_prob = F.softmax(logits, dim=1)  
        return logits, Y_prob, Y_hat, A_raw, outputs  
