...
def objective(trial, datasets, it):
    # Generate the optimizers.
    reg = trial.suggest_int("reg", -8, 0, step= 1)
    if reg == 0:
        args.reg = 0.2
    else:
        args.reg = 10**reg

    #lr = trial.suggest_int("lr", -5, -3, step= 1)
    #args.lr = 10**lr

    # Training of the model.
    _, test_auc, val_auc, test_acc, val_acc, test_loss, val_loss  = train(datasets, it, args)
    return val_loss
...
def objective_cv(trial):
    ...
    for it in folds:        
        ...
        train_dataset, val_dataset, test_dataset = dataset.return_splits(from_id=False,
                csv_path='{}/splits_{}.csv'.format(args.split_dir, it))
        datasets = (train_dataset, val_dataset, test_dataset)

        loss = objective(trial, datasets, it)
        losses.append(loss)
...
if __name__ == "__main__":
    search_space = {"reg": [-8, -7, -6, -5, -4, -3, -2, -1, 0]}
    study = optuna.create_study(sampler=optuna.samplers.GridSampler(search_space), direction="minimize")
    study.optimize(objective_cv, n_trials= 4000, timeout= 1000000000000000)
